...
#define DRAW_RECT_COUNT 10
#define DRAW_RECT_RGN_HANDLE 20
#define DRAW_RECT_OVERLAY_LAYER 7
#define DRAW_RECT_ITEM_W 112  /* single rect width, 16-aligned */
#define DRAW_RECT_ITEM_H 112  /* single rect height, 16-aligned */
#define DRAW_RECT_BORDER 4
#define DRAW_RECT_COLOR_ARGB 0xFF00FF00 /* opaque green */

#define DRAW_RGA_RECT_COUNT 3
#define DRAW_RGA_RGN_HANDLE 21
#define DRAW_RGA_OVERLAY_LAYER 6
#define DRAW_RGA_RECT_W 112  /* 16-aligned */
#define DRAW_RGA_RECT_H 112  /* 16-aligned */
#define DRAW_RGA_BORDER 4
#define DRAW_RGA_COLOR_ARGB 0xFFFF0000 /* opaque red */
...
static int g_display_camera_index = 0;
static pthread_mutex_t g_display_camera_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static pthread_t venc_thread_0, venc_thread_1, venc_thread_2, get_jpeg_thread_id, vpss_thread_rgb,
    get_vi_send_jpeg_thread_id, cycle_snapshot_thread_id, draw_rect_thread_id,
    draw_rga_rect_thread_id;
...
MPP_CHN_S vo_chn, vpss_to_npu_chn;
MPP_CHN_S vi_chn[MAX_RKIPC_SENSOR_NUM], venc_chn[MAX_RKIPC_VENC_NUM],
    vpss_chn[MAX_RKIPC_VENC_NUM];
MPP_CHN_S display_vpss_chn;
MPP_CHN_S encode_src_chn;

static VO_DEV VoLayer = RK3588_VOP_LAYER_CLUSTER0;

static int rkipc_vo_bind_camera_locked(int camera_index) {
	int ret;
	MPP_CHN_S target_vi = vi_chn[camera_index];
	ret = RK_MPI_SYS_Bind(&target_vi, &display_vpss_chn);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("bind error %#x: vi [%d, %d] -> display vpss [%d, %d]\n", ret, target_vi.s32DevId,
		          target_vi.s32ChnId, display_vpss_chn.s32DevId, display_vpss_chn.s32ChnId);
		return ret;
	}
	g_display_camera_index = camera_index;
	LOG_INFO("display camera switched to %d\n", g_display_camera_index + 1);
	return RK_SUCCESS;
}

static int rkipc_encode_bind_camera_locked(int camera_index) {
	int ret;
	MPP_CHN_S target_vi = vi_chn[camera_index];
	ret = RK_MPI_SYS_Bind(&target_vi, &encode_src_chn);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("bind error %#x: vi [%d, %d] -> encode src [%d, %d]\n", ret, target_vi.s32DevId,
		          target_vi.s32ChnId, encode_src_chn.s32DevId, encode_src_chn.s32ChnId);
		return ret;
	}
	return RK_SUCCESS;
}
...
int rkipc_vo_init() {
	...
	RK_U32 width = rk_param_get_int("avs:source_width", -1);
	RK_U32 height = rk_param_get_int("avs:source_height", -1);
...
int rkipc_display_vpss_init() {
	...
	memset(&stVpssGrpAttr, 0, sizeof(stVpssGrpAttr));
	memset(&stVpssChnAttr[0], 0, sizeof(stVpssChnAttr[0]));
	stVpssGrpAttr.u32MaxW = rk_param_get_int("avs:source_width", 3840);
	stVpssGrpAttr.u32MaxH = rk_param_get_int("avs:source_height", 2160);
	stVpssGrpAttr.stFrameRate.s32SrcFrameRate = -1;
	stVpssGrpAttr.stFrameRate.s32DstFrameRate = -1;
	stVpssGrpAttr.enPixelFormat = RK_FMT_YUV420SP;
	if (g_format)
	...
	stVpssChnAttr[0].stFrameRate.s32SrcFrameRate = -1;
	stVpssChnAttr[0].stFrameRate.s32DstFrameRate = -1;
	stVpssChnAttr[0].u32Width = rk_param_get_int("avs:source_width", 3840);
	stVpssChnAttr[0].u32Height = rk_param_get_int("avs:source_height", 2160);
	if (g_format)
		stVpssChnAttr[0].enCompressMode = COMPRESS_AFBC_16x16;
	ret = RK_MPI_VPSS_SetChnAttr(VpssGrp, VpssChn[0], &stVpssChnAttr[0]);
	if (ret != RK_SUCCESS)
...
int rkipc_bind_init() {
	...
	encode_src_chn = vpss_chn[0];
	pthread_mutex_lock(&g_display_camera_mutex);
	ret = rkipc_encode_bind_camera_locked(g_display_camera_index);
	pthread_mutex_unlock(&g_display_camera_mutex);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("bind error %#x: vi [%d] -> vpss_chn[0] [%d, %d]", ret, g_display_camera_index,
		          vpss_chn[0].s32DevId, vpss_chn[0].s32ChnId);
		return ret;
	}
	...
	if (g_enable_vo) {
		...
		vo_chn.s32DevId = 0;
		vo_chn.s32ChnId = 0;
		display_vpss_chn.enModId = RK_ID_VPSS;
		display_vpss_chn.s32DevId = 3;
		display_vpss_chn.s32ChnId = 0;
		ret = RK_MPI_SYS_Bind(&display_vpss_chn, &vo_chn);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("bind error %#x: display_vpss_chn [%d, %d] -> vo [%d, %d]", ret,
			          display_vpss_chn.s32DevId, display_vpss_chn.s32ChnId, vo_chn.s32DevId,
			          vo_chn.s32ChnId);
			// return ret;
		}
		pthread_mutex_lock(&g_display_camera_mutex);
		ret = rkipc_vo_bind_camera_locked(g_display_camera_index);
		pthread_mutex_unlock(&g_display_camera_mutex);
		if (ret != RK_SUCCESS)
			return ret;
	}
...
int rkipc_bind_deinit() {
	...
	if (g_enable_vo) {
		pthread_mutex_lock(&g_display_camera_mutex);
		ret = RK_MPI_SYS_UnBind(&vi_chn[g_display_camera_index], &display_vpss_chn);
		pthread_mutex_unlock(&g_display_camera_mutex);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("unbind error %#x: vi [%d, %d] -> display_vpss_chn [%d, %d]", ret,
			          vi_chn[g_display_camera_index].s32DevId, vi_chn[g_display_camera_index].s32ChnId,
			          display_vpss_chn.s32DevId, display_vpss_chn.s32ChnId);
		}
		ret = RK_MPI_SYS_UnBind(&display_vpss_chn, &vo_chn);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("unbind error %#x: display_vpss_chn [%d, %d] -> vo [%d, %d]", ret,
			          display_vpss_chn.s32DevId, display_vpss_chn.s32ChnId, vo_chn.s32DevId,
			          vo_chn.s32ChnId);
		}
	}
	...
	pthread_mutex_lock(&g_display_camera_mutex);
	ret = RK_MPI_SYS_UnBind(&vi_chn[g_display_camera_index], &encode_src_chn);
	pthread_mutex_unlock(&g_display_camera_mutex);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("unbind error %#x: vi [%d] -> encode src [%d, %d]", ret, g_display_camera_index,
		          encode_src_chn.s32DevId, encode_src_chn.s32ChnId);
	}
	LOG_INFO("end\n");
...
int rk_video_get_display_camera(int *camera_index) {
	if (!camera_index)
		return -1;
	pthread_mutex_lock(&g_display_camera_mutex);
	*camera_index = g_display_camera_index;
	pthread_mutex_unlock(&g_display_camera_mutex);
	return 0;
}

int rk_video_set_display_camera(int camera_index) {
	int ret = 0;
	if (camera_index < 0 || camera_index >= g_sensor_num) {
		LOG_ERROR("invalid camera index: %d, sensor num: %d\n", camera_index, g_sensor_num);
		return -1;
	}
	pthread_mutex_lock(&g_display_camera_mutex);
	if (camera_index == g_display_camera_index) {
		pthread_mutex_unlock(&g_display_camera_mutex);
		return 0;
	}
	ret = RK_MPI_SYS_UnBind(&vi_chn[g_display_camera_index], &encode_src_chn);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("unbind error %#x: vi [%d, %d] -> encode src [%d, %d]\n", ret,
		          vi_chn[g_display_camera_index].s32DevId, vi_chn[g_display_camera_index].s32ChnId,
		          encode_src_chn.s32DevId, encode_src_chn.s32ChnId);
		pthread_mutex_unlock(&g_display_camera_mutex);
		return ret;
	}
	ret = rkipc_encode_bind_camera_locked(camera_index);
	if (ret != RK_SUCCESS) {
		pthread_mutex_unlock(&g_display_camera_mutex);
		return ret;
	}
	...
	ret = RK_MPI_SYS_UnBind(&vi_chn[g_display_camera_index], &display_vpss_chn);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("unbind error %#x: vi [%d, %d] -> display vpss [%d, %d]\n", ret,
		          vi_chn[g_display_camera_index].s32DevId, vi_chn[g_display_camera_index].s32ChnId,
		          display_vpss_chn.s32DevId, display_vpss_chn.s32ChnId);
		pthread_mutex_unlock(&g_display_camera_mutex);
		return ret;
	}
	ret = rkipc_vo_bind_camera_locked(camera_index);
	pthread_mutex_unlock(&g_display_camera_mutex);
	return ret;
}
...
static int g_draw_rect_overlay_w = 0;
static int g_draw_rect_overlay_h = 0;

/*
 * Draw 10 rectangles on the full-frame overlay bitmap using draw_border().
 * Prints the elapsed time for the CPU drawing operation.
 */
static void rkipc_draw_rects_on_buf(uint32_t *buf, int overlay_w, int overlay_h,
                                    int venc_w, int venc_h) {
	struct timespec ts_start, ts_end;
	clock_gettime(CLOCK_MONOTONIC, &ts_start);

	/* Clear the entire overlay to transparent */
	memset(buf, 0, overlay_w * overlay_h * 4);

	/*
	 * Layout: 10 rectangles in 2 rows x 5 columns, evenly distributed.
	 * Row 0: y = 1/3 of frame height
	 * Row 1: y = 2/3 of frame height
	 */
	...
	for (int i = 0; i < DRAW_RECT_COUNT; i++) {
		int col = i % 5;
		int row = i / 5;
		int cx = venc_w * (col + 1) / 6;          /* center x for this column */
		int cy = venc_h * (row + 1) / 3;           /* center y for this row */
		int rx = cx - DRAW_RECT_ITEM_W / 2;
		int ry = cy - DRAW_RECT_ITEM_H / 2;

		/* Clamp to overlay bounds */
		if (rx < 0) rx = 0;
		if (ry < 0) ry = 0;
		if (rx + DRAW_RECT_ITEM_W > overlay_w) rx = overlay_w - DRAW_RECT_ITEM_W;
		if (ry + DRAW_RECT_ITEM_H > overlay_h) ry = overlay_h - DRAW_RECT_ITEM_H;

		/*
		 * draw_border() draws on a standalone w*h buffer.
		 * To draw at an offset in a larger buffer, we use a temp buffer
		 * and then copy rows into the overlay buffer at the right position.
		 */
		uint32_t temp_buf[DRAW_RECT_ITEM_W * DRAW_RECT_ITEM_H];
		memset(temp_buf, 0, sizeof(temp_buf));
		border_info.rect.x = 0;
		border_info.rect.y = 0;
		border_info.rect.w = DRAW_RECT_ITEM_W;
		border_info.rect.h = DRAW_RECT_ITEM_H;
		draw_border(temp_buf, border_info);

		/* Blit temp_buf into the overlay at (rx, ry) */
		for (int y = 0; y < DRAW_RECT_ITEM_H; y++) {
			for (int x = 0; x < DRAW_RECT_ITEM_W; x++) {
				uint32_t pixel = temp_buf[y * DRAW_RECT_ITEM_W + x];
				if (pixel != 0) { /* only write non-transparent pixels */
					buf[(ry + y) * overlay_w + (rx + x)] = pixel;
				}
			}
		}
	}
	...
	LOG_INFO("draw_rect: drew %d rectangles on %dx%d overlay in %ld us\n",
	         DRAW_RECT_COUNT, overlay_w, overlay_h, elapsed_us);
}
...
static void *rkipc_draw_rect_thread(void *arg) {
	...
	BITMAP_S stBitmap;
	stBitmap.enPixelFormat = RK_FMT_ARGB8888;
	stBitmap.u32Width = g_draw_rect_overlay_w;
	stBitmap.u32Height = g_draw_rect_overlay_h;
	stBitmap.pData = (RK_VOID *)g_draw_rect_buf;

	/* Periodically redraw and refresh the bitmap */
	while (g_video_run_) {
		rkipc_draw_rects_on_buf(g_draw_rect_buf, g_draw_rect_overlay_w,
		                        g_draw_rect_overlay_h, venc_w, venc_h);
...
static int rkipc_draw_rect_init() {
	...
	/* Overlay covers the full frame, dimensions must be 16-aligned */
	g_draw_rect_overlay_w = UPALIGNTO16(venc_w);
	g_draw_rect_overlay_h = UPALIGNTO16(venc_h);

	/* Allocate ARGB8888 bitmap for the full-frame overlay */
	int buf_size = g_draw_rect_overlay_w * g_draw_rect_overlay_h * 4;
	...
	/* Draw all 10 rectangles on the bitmap */
	rkipc_draw_rects_on_buf(g_draw_rect_buf, g_draw_rect_overlay_w,
	                        g_draw_rect_overlay_h, venc_w, venc_h);

	/* Create the OVERLAY region covering the full frame */
	RGN_ATTR_S stRgnAttr;
	memset(&stRgnAttr, 0, sizeof(stRgnAttr));
	stRgnAttr.enType = OVERLAY_RGN;
	stRgnAttr.unAttr.stOverlay.enPixelFmt = RK_FMT_ARGB8888;
	stRgnAttr.unAttr.stOverlay.stSize.u32Width = g_draw_rect_overlay_w;
	stRgnAttr.unAttr.stOverlay.stSize.u32Height = g_draw_rect_overlay_h;
	...
	LOG_INFO("draw_rect: RGN handle %d created, overlay %dx%d\n",
	         DRAW_RECT_RGN_HANDLE, g_draw_rect_overlay_w, g_draw_rect_overlay_h);

	/* Attach overlay at (0,0) on VENC channel 1 (live/1) */
	MPP_CHN_S stMppChn;
	stMppChn.enModId = RK_ID_VENC;
	stMppChn.s32DevId = 0;
	...
	LOG_INFO("draw_rect: attached to VENC ch%d at (0,0)\n", VIDEO_PIPE_1);
	...
	if (ret != RK_SUCCESS) {
		...
		LOG_INFO("draw_rect: initial bitmap set OK\n");
	}
...
/*
 * RGA-based rectangle drawing: uses RGA imfill() to draw 3 red rectangles
 * on a full-frame overlay, then sets as RGN bitmap on VENC channel 1.
 */
static uint32_t *g_draw_rga_buf = NULL;
static int g_draw_rga_overlay_w = 0;
static int g_draw_rga_overlay_h = 0;

static void rkipc_draw_rga_rects_on_buf(uint32_t *buf, int overlay_w, int overlay_h,
                                         int venc_w, int venc_h) {
	struct timespec ts_start, ts_end;
	clock_gettime(CLOCK_MONOTONIC, &ts_start);
	...
	rga_buffer_t dst = wrapbuffer_virtualaddr(buf, overlay_w, overlay_h, RK_FORMAT_BGRA_8888);

	/*
	 * 3 rectangles: left, center, right at vertical center.
	 * For each rectangle, draw 4 filled edge strips using imfill().
	 */
	int positions[DRAW_RGA_RECT_COUNT][2] = {
	    {venc_w / 4 - DRAW_RGA_RECT_W / 2, (venc_h - DRAW_RGA_RECT_H) / 2},
	    {(venc_w - DRAW_RGA_RECT_W) / 2, (venc_h - DRAW_RGA_RECT_H) / 2},
	    {venc_w * 3 / 4 - DRAW_RGA_RECT_W / 2, (venc_h - DRAW_RGA_RECT_H) / 2},
	};

	for (int i = 0; i < DRAW_RGA_RECT_COUNT; i++) {
		int rx = positions[i][0];
		int ry = positions[i][1];
		int rw = DRAW_RGA_RECT_W;
		int rh = DRAW_RGA_RECT_H;
		int t = DRAW_RGA_BORDER;
		IM_STATUS status;

		/* Top edge */
		im_rect top = {rx, ry, rw, t};
		status = imfill(dst, top, DRAW_RGA_COLOR_ARGB);
		if (status != IM_STATUS_SUCCESS)
			LOG_ERROR("draw_rga[%d] top imfill failed: %s\n", i, imStrError(status));

		/* Bottom edge */
		im_rect bottom = {rx, ry + rh - t, rw, t};
		status = imfill(dst, bottom, DRAW_RGA_COLOR_ARGB);
		if (status != IM_STATUS_SUCCESS)
			LOG_ERROR("draw_rga[%d] bottom imfill failed: %s\n", i, imStrError(status));

		/* Left edge */
		im_rect left = {rx, ry + t, t, rh - 2 * t};
		status = imfill(dst, left, DRAW_RGA_COLOR_ARGB);
		if (status != IM_STATUS_SUCCESS)
			LOG_ERROR("draw_rga[%d] left imfill failed: %s\n", i, imStrError(status));

		/* Right edge */
		im_rect right = {rx + rw - t, ry + t, t, rh - 2 * t};
		status = imfill(dst, right, DRAW_RGA_COLOR_ARGB);
		if (status != IM_STATUS_SUCCESS)
			LOG_ERROR("draw_rga[%d] right imfill failed: %s\n", i, imStrError(status));
	}

	clock_gettime(CLOCK_MONOTONIC, &ts_end);
	long elapsed_us = (ts_end.tv_sec - ts_start.tv_sec) * 1000000L +
	                  (ts_end.tv_nsec - ts_start.tv_nsec) / 1000L;
	LOG_INFO("draw_rga: drew %d red rectangles on %dx%d overlay in %ld us\n",
	         DRAW_RGA_RECT_COUNT, overlay_w, overlay_h, elapsed_us);
}

static void *rkipc_draw_rga_rect_thread(void *arg) {
	(void)arg;
	LOG_INFO("#Start %s thread\n", __func__);
	prctl(PR_SET_NAME, "rkipc_rga_rect");
	...
	BITMAP_S stBitmap;
	stBitmap.enPixelFormat = RK_FMT_ARGB8888;
	stBitmap.u32Width = g_draw_rga_overlay_w;
	stBitmap.u32Height = g_draw_rga_overlay_h;
	stBitmap.pData = (RK_VOID *)g_draw_rga_buf;

	while (g_video_run_) {
		rkipc_draw_rga_rects_on_buf(g_draw_rga_buf, g_draw_rga_overlay_w,
		                            g_draw_rga_overlay_h, venc_w, venc_h);
		int ret = RK_MPI_RGN_SetBitMap(DRAW_RGA_RGN_HANDLE, &stBitmap);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("draw_rga: RK_MPI_RGN_SetBitMap refresh failed %#x\n", ret);
		}
		usleep(500 * 1000);
	}

	LOG_INFO("draw_rga: thread exiting\n");
	return NULL;
}

static int rkipc_draw_rga_rect_init() {
	LOG_INFO("start\n");
	int ret;
	int venc_w = rk_param_get_int("video.1:width", 1920);
	int venc_h = rk_param_get_int("video.1:height", 1088);

	g_draw_rga_overlay_w = UPALIGNTO16(venc_w);
	g_draw_rga_overlay_h = UPALIGNTO16(venc_h);

	int buf_size = g_draw_rga_overlay_w * g_draw_rga_overlay_h * 4;
	g_draw_rga_buf = (uint32_t *)malloc(buf_size);
	if (!g_draw_rga_buf) {
		LOG_ERROR("draw_rga: malloc %d bytes failed\n", buf_size);
		return -1;
	}
	...
	stRgnAttr.enType = OVERLAY_RGN;
	stRgnAttr.unAttr.stOverlay.enPixelFmt = RK_FMT_ARGB8888;
	stRgnAttr.unAttr.stOverlay.stSize.u32Width = g_draw_rga_overlay_w;
	stRgnAttr.unAttr.stOverlay.stSize.u32Height = g_draw_rga_overlay_h;
	ret = RK_MPI_RGN_Create(DRAW_RGA_RGN_HANDLE, &stRgnAttr);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("draw_rga: RK_MPI_RGN_Create(%d) failed %#x\n", DRAW_RGA_RGN_HANDLE, ret);
		free(g_draw_rga_buf);
		g_draw_rga_buf = NULL;
		return -1;
	}
	LOG_INFO("draw_rga: RGN handle %d created, overlay %dx%d\n",
	         DRAW_RGA_RGN_HANDLE, g_draw_rga_overlay_w, g_draw_rga_overlay_h);

	/* Attach overlay at (0,0) on VENC channel 1 */
	MPP_CHN_S stMppChn;
	stMppChn.enModId = RK_ID_VENC;
	stMppChn.s32DevId = 0;
	stMppChn.s32ChnId = VIDEO_PIPE_1;
	...
	stRgnChnAttr.unChnAttr.stOverlayChn.u32BgAlpha = 128;
	stRgnChnAttr.unChnAttr.stOverlayChn.u32FgAlpha = 128;
	stRgnChnAttr.unChnAttr.stOverlayChn.u32Layer = DRAW_RGA_OVERLAY_LAYER;

	ret = RK_MPI_RGN_AttachToChn(DRAW_RGA_RGN_HANDLE, &stMppChn, &stRgnChnAttr);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("draw_rga: RK_MPI_RGN_AttachToChn to venc%d failed %#x\n", VIDEO_PIPE_1, ret);
		RK_MPI_RGN_Destroy(DRAW_RGA_RGN_HANDLE);
		free(g_draw_rga_buf);
		g_draw_rga_buf = NULL;
		return -1;
	}
	...
	stBitmap.u32Height = g_draw_rga_overlay_h;
	stBitmap.pData = (RK_VOID *)g_draw_rga_buf;
	ret = RK_MPI_RGN_SetBitMap(DRAW_RGA_RGN_HANDLE, &stBitmap);
	if (ret != RK_SUCCESS) {
		LOG_ERROR("draw_rga: RK_MPI_RGN_SetBitMap failed %#x\n", ret);
	} else {
		LOG_INFO("draw_rga: initial bitmap set OK\n");
	}

	/* Start refresh thread */
	pthread_create(&draw_rga_rect_thread_id, NULL, rkipc_draw_rga_rect_thread, NULL);
	LOG_INFO("end\n");
	return 0;
}

static int rkipc_draw_rga_rect_deinit() {
	LOG_INFO("start\n");
	pthread_join(draw_rga_rect_thread_id, NULL);

	MPP_CHN_S stMppChn;
	stMppChn.enModId = RK_ID_VENC;
	stMppChn.s32DevId = 0;
	stMppChn.s32ChnId = VIDEO_PIPE_1;
	RK_MPI_RGN_DetachFromChn(DRAW_RGA_RGN_HANDLE, &stMppChn);
	RK_MPI_RGN_Destroy(DRAW_RGA_RGN_HANDLE);
...
int rk_video_init() {
	...
	g_display_camera_index = rk_param_get_int("avs:display_camera_index", 0);
	if (g_display_camera_index < 0 || g_display_camera_index >= g_sensor_num)
		g_display_camera_index = 0;
	...
	ret |= rkipc_osd_init();
	if (enable_venc_1) {
		ret |= rkipc_draw_rect_init();
		ret |= rkipc_draw_rga_rect_init();
	}
...
int rk_video_deinit() {
	...
	g_video_run_ = 0;
	if (enable_venc_1) {
		ret |= rkipc_draw_rga_rect_deinit();
		ret |= rkipc_draw_rect_deinit();
	}
