...
post_chk()
{
	...
	if [ "$result"x == "2"x ] ;then
		ln -s -f /usr/share/rkipc-2x.ini $default_rkipc_ini
	fi

	# Fallback to 2x when sensor count detection is abnormal.
...
# 确保 debugfs 已挂载，否则无法设置 ISP 频率
mount -t debugfs none /sys/kernel/debug 2>/dev/null || true
# ISP 频率设置，增加判断防止报错
if [ -e /sys/kernel/debug/clk/clk_isp0_core/clk_rate ]; then
    echo 702000000 > /sys/kernel/debug/clk/clk_isp0_core/clk_rate 2>/dev/null || echo "Warning: ISP0 clk write denied"
    echo 702000000 > /sys/kernel/debug/clk/clk_isp1_core/clk_rate 2>/dev/null || echo "Warning: ISP1 clk write denied"
fi
